...
class  QuienesSomos  extends Component <ActividadesProps, ActividadesState>{
  ...
  render() {
    ...
    const renderActividadesItem = ({ item, index }: { item: Actividad; index: number }) => {
        return (
          <ListItem
            key={index}
    ...
    return (
        <Card {...cardProps}>
            ...
            <SafeAreaView>
                 <FlatList data={this.state.actividades} renderItem={renderActividadesItem} keyExtractor={(item) => item.id.toString()} />
            </SafeAreaView>
